...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	if (gScriptLoader.OnCommandManagerDirect(lpObj->Index, message) != 0)
	{
		return 0;
	}
...
void CCommandManager::DiscountRequirement(LPOBJ lpObj, int index) // OK
{
	...
	LogAdd(LOG_BLUE, "DiscountRequirement > Codigo %d", index);
...
bool CCommandManager::CommandSet(LPOBJ lpObj,char* arg) // OK
{
	...
	for(int n=0;n < socket && socket <= MAX_SOCKET_OPTION;n++)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(901));
