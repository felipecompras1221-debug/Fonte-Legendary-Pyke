...
{	
	...
	float startX = (640.0 - getXX(329)) / 2, startY = (480.0 - getYY(245)) * 2.0 / 3.0;
...
// - Campo de Login e Senha
	if (*(DWORD*)0xE60960 == 1)
	{
		SetBoundInputField(*(DWORD *)(Struct + 848),startX + getXX(114) + PosXLoginAndSenha, startY + getYY(56) + Baixar + PointSY1 + PosYLoginAndSenha);
		SetBoundInputField(*(DWORD *)(Struct + 852),startX + getXX(114) + PosXLoginAndSenha, startY + getYY(88) + Baixar + PointSY1 + PosYLoginAndSenha + PosPart1Y);
	}
	...
	RenderBitmap(95124, 195.0, 200.5 + Baixar + PointSY1, 250.0, 216.0, 0, 0, 0.9765625, 0.84375, 1, 1, 0.0); //LoginBox

	if(*(int*)0x87935A4 == 2) //Tela de Login
	{	
		if (SEASON3B_CheckMouseIn(326, 351, 67, 15))	
		{
			RenderBitmap(95125, 324.0, 295.0 + Baixar + PointSY1, 70.0, 19.0, 0, 0, 0.546875, 0.59375, 1, 1, 0.0); //Connect
		}

		if (SEASON3B_CheckMouseIn(287, 436, 67, 15))	
		{
			RenderBitmap(95126, 285.0, 380.5 + Baixar, 70.0, 20.0, 0, 0, 0.546875, 0.625, 1, 1, 0.0);
		}
	}
	...
	//pDrawText(pTextThis(), startX + getXX(124), startY + getYY(158) + Baixar, "Save password", getXX(120), 0, (PINT)4, 0);
	pDrawText(pTextThis(), startX + getXX(142) + PosXConnect, startY + getYY(134) + Baixar + PointSY1 + PosYConnect, gCustomMessage.GetMessageB(452)/*Connect*/, getXX(120), 0, (PINT)4, 0);
	pDrawText(pTextThis(), startX - getXX(93) + PosXLoginAndSenha2, startY + getYY(50) + Baixar + PointSY1 + PosYLoginAndSenha, gCustomMessage.GetMessageB(450)/*Login*/, getXX(120), 0, (PINT)4, 0);
	pDrawText(pTextThis(), startX - getXX(90) + PosXLoginAndSenha2, startY + getYY(85) + Baixar + PointSY1 + PosYLoginAndSenha + PosPart1Y, gCustomMessage.GetMessageB(451)/*Senha*/, getXX(120), 0, (PINT)4, 0);
	pDrawText(pTextThis(), startX + getXX(55) + PosXExit, startY + getYY(270) + Baixar + PointSY1 + PosYExit, gCustomMessage.GetMessageB(453)/*Exit*/, getXX(120), 0, (PINT)4, 0);
		
	//pDrawText(pTextThis(), startX - getXX(102), startY - getYY(50) + Baixar + PointSY1, Testando, getXX(120), 0, (PINT)4, 0);pSetFont(pTextThis(), *(DWORD*)0xE8C588);
...
{
	SetDword(0x006311B6+1, (DWORD)"Custom\\Interface\\none.tga");		//Remoção Interface LoginBox
	SetDword(0x006311D6+1, (DWORD)"Custom\\Interface\\none.tga");		//Remoção Interface LoginBox
	...
	//MemorySet(0x00401F56,0x90,0x5);                                     //Remoção Botão do commoner escrita
}