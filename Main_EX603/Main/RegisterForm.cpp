...
BYTE DrawCButton2(int IMGcode, float x, float y, float w, float h, float a6 = 64.0, float a7 = 128.0, int delay = 150, bool a9 = false) 
{
	if (IsCursorInZone1(x, y, getX1(w), getY1(h)))
	{
		...
		{
			aRenderBitmap(IMGcode, x, y, w, h, 0.0, h / a7, w / a6, h / a7, 0, 1, 0.0);
		}
		else 
		{
			aRenderBitmap(IMGcode, x, y, w, h, 0.0, 0.0, w / a6, h / a7, 0, 1, 0.8);
		}
	...
	{
		aRenderBitmap(IMGcode, x, y, w, h, 0.0, 0.0, w / a6, h / a7, 0, 1, 0.0);
	}
...
{
	if(FindText2(szID, " ", 0) || FindText2(szID,"¡¡", 0) || FindText2(szID, ".", 0) || FindText2(szID,"¡¤", 0) || FindText2(szID, "¡­", 0) || FindText2(szID,"Webzen", 0) || FindText2(szID, "WebZen", 0) || FindText2(szID,"webzen", 0) || FindText2(szID, "WEBZEN", 0) || FindText2(szID,GlobalText(GlobalLine,457), 0) || FindText2(szID, GlobalText(GlobalLine,458), 0))
	{
		return true;
	}
...
void CRegisterForm::Create(BYTE* thisa)
{
	...
	sprInputBoxEmail->Init(g_hWnd, 80, 14, 50);
...
void CRegisterForm::SetPosicion(int x, int y)
{
	...
	sprInputBoxCaptcha->SetPosition(x + 126, y + 135);
}
...
void CRegisterForm::RenderForm()
{
	...
	CRenderBitmaps(71631, (float)x, (float)y, 220.f, 300.f, 0.0, 0.0, 190.f/256.f, 250.f/256.f, true, true, 0.0);
	...
	g_pRenderText2.RenderText(x - 32, y + 62, GlobalText(GlobalLine, 450), 93, 0, RT3_SORT_RIGHT);
	g_pRenderText2.RenderText(x - 32, y + 82, GlobalText(GlobalLine, 451), 93, 0, RT3_SORT_RIGHT);
	g_pRenderText2.RenderText(x - 32, y + 99, "E-mail ", 93, 0, RT3_SORT_RIGHT);
	g_pRenderText2.RenderText(x - 32, y + 117, "Personal Code ", 93, 0, RT3_SORT_RIGHT);
	g_pRenderText2.RenderText(x - 32, y + 136, " Captcha ", 93, 0, RT3_SORT_RIGHT);
	g_pRenderText2.SetFont(g_hFont);
	g_pRenderText2.RenderText(x + 80, y + 136, this->reCaptcha2);
	...
	RenderUI2DButton1(0x7A5B, (float)x + 64, (float)y + 156, 54.0, 30.0, 13.0, 15.0, 16.0, 64.0, RequestRegister);

	RenderUI2DButton1(0x7A5C, (float)x + 120,(float)y + 156, 54.0, 30.0, 13.0 , 15.0, 16.0, 64.0, CloseFormRegister);
...
void CRegisterForm::RenderUI2DButton1(int IMGcode, float x, float y, float w, float h, float uw, float uh, float a8, float a9, PUSHEVENT_CALLBACK_LPVOID pCallbackFunc)
{
	if (IsCursorInZone1((int)x, (int)y,(int)w,(int)h))
	{
		if (GetKeyState(VK_LBUTTON) & 0x8000)
		{
			aRenderBitmap(IMGcode, x, y, w, h, 0.0, uh / a9 * 2.0, uw / a8, uh / a9, 0, 1, 0.0);
		}
		else 
		{
			aRenderBitmap(IMGcode, x, y, w, h, 0.0, uh / a9, uw / a8, uh / a9, 0, 1, 0.0);
		}
	...
	{
		aRenderBitmap(IMGcode, x, y, w, h, 0.0, 0.0, uw / a8, uh / a9, 0, 1, 0.0);
	}
...
void CRegisterForm::RequestRegister(LPVOID pClass)
{
	...
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Please enter the captcha code!");
	}
	else if(strcmp(szth->reCaptcha, szCaptcha) != NULL)
	{
		szth->CodeAction = CREATE_ACCOUNT_FAIL_CAPTCHA;
		PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Capcha code is not correct!");
	}
	else
	{
		if(CheckName(szID))
...
void CRegisterForm::RequestAccount(BYTE code)
{
	...
	switch(code)
	{
		case CREATE_ACCOUNT_FAIL_ID:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Username already registered");
			break;
		case CREATE_ACCOUNT_SUCCESS:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "Account Registration Successfully");
			break;
		case CREATE_ACCOUNT_FAIL_RESIDENT:
			PopUpMsgWin(CharMakeWin(), MESSAGE_FREE_MSG_NOT_BTN, "The account creation system is temporarily unavailable");
			break;
		default:
