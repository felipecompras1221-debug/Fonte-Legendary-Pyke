...
void CAchievements::Draw()
{
	...
	for (int i = 0; i < ACHIEVEMENTS_COUNT; i++)
	{
		if (gInterface.IsWorkZone2(RightX, DownY, RightX + 32, DownY + 32))
		{
			...
			gNewInterface.TextDraw(TooltipX, TooltipY, 0, 1, eGold, 0, 8, pFontBold, this->m_SettingsData[i].Name);

			gNewInterface.TextDraw(TooltipX, TooltipY + 11, 0, 1, eWhite, 0, 8, pFontNormal, "%d / %d", this->m_UserCount[i], this->m_SettingsData[i].counter);

			gNewInterface.TextDraw(TooltipX, TooltipY + 11 + 10, 0, 1, eWhite, 0, 8, pFontNormal, "Reward: %d %s", this->m_SettingsData[i].RewardValue, GetRewardTextByType(this->m_SettingsData[i].RewardType));
		}
	...
	if (gInterface.IsWorkZone2(270, 13, 373, 38))
	{
		...
		barStartY += 5;
		gNewInterface.TextDraw(barStartX + barWidth / 2, barStartY, 0, 1, eGold, 0, 8, pFontBold, "[Achievements Power]");

		barStartY += 15;
		gNewInterface.TextDraw(barStartX + barWidth / 2, barStartY, 0, 1, eWhite, 0, 8, pFontBold, "Damage: %d / %d", this->m_Damage, this->m_DamageMax);

		barStartY += 10;
		gNewInterface.TextDraw(barStartX + barWidth / 2, barStartY, 0, 1, eWhite, 0, 8, pFontBold, "Defense: %d / %d", this->m_Defence, this->m_DefenceMax);

		barStartY += 10;
		gNewInterface.TextDraw(barStartX + barWidth / 2, barStartY, 0, 1, eWhite, 0, 8, pFontBold, "Life: %d / %d", this->m_Life, this->m_LifeMax);

		barStartY += 10;
		gNewInterface.TextDraw(barStartX + barWidth / 2, barStartY, 0, 1, eWhite, 0, 8, pFontBold, "Excellent Damage: %d / %d", this->m_ExellentDamage, this->m_ExellentDamageMax);

		barStartY += 10;
		gNewInterface.TextDraw(barStartX + barWidth / 2, barStartY, 0, 1, eWhite, 0, 8, pFontBold, "Critical Damage: %d / %d", this->m_CriticalDamage, this->m_CriticalDamageMax);
	}
}
...
void CAchievements::CGWindowOpen()
{
	...
	pMsg.h.set(0xFF, 0x0C, sizeof(pMsg));
