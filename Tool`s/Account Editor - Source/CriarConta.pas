...
begin
  if Trim(Edit1.Text ) = '' then begin
   MessageBox(Handle, text13, text04, MB_ICONERROR or MB_OK);
   Exit;
  end;

  Cn                          := TFDConnection.Create(nil);
  Cn.ConnectionName           := 'ConexaoMSSQL2';
  Cn.DriverName               := 'MSSQL';
  ...
  try
    ...
    try
      Query.Connection                          := cn;
      Query.SQL.Text                            := 'SELECT top 1 memb___id FROM MEMB_INFO WHERE memb___id = :conta';
      Query.Params.ParamByName('conta').Value   := Edit1.Text;
      Query.Active                              := True;

        if Query.RowsAffected > 0 then begin
            MessageBox(Handle, text11, text04, MB_ICONERROR or MB_OK);
        end
        else begin
            CriarConta();
        end;
...
begin
  ...
  try
    ...
    try
      Query.Connection                            := cn;
      Query.SQL.Text                              := 'INSERT INTO MEMB_INFO (memb___id,memb__pwd,memb_name,mail_addr,appl_days,mail_chek,bloc_code,ctl1_code,sno__numb) VALUES (:conta,:senha,:name,:mail,(getdate()),1,0,1,1)';
      ...
      Query.ExecSQL;
        ...
        if Query.RowsAffected > 0 then begin
            ...
            Edit3.Text := '';
            Edit4.Text := '';
            MessageBox(Handle, text12, text03, MB_ICONINFORMATION or MB_OK);
        end
        else begin
