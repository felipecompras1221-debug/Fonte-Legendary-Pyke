...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	this->m_brush[1] = CreateSolidBrush(RGB(0, 255, 0));	//active verde
	this->m_brush[2] = CreateSolidBrush(RGB(21, 89, 202));	//roxo 
...
void CServerDisplayer::PaintInvasionTime() // OK
{
	...
	for (int n=0; n < 26; n++)
	{
		...
		if (n==2) { wsprintf(text1,"%d - %s",n, "Golden: "); }
		...
		if (n==9) { wsprintf(text1,"%d - %s",n, "Narcondra: "); }
		if (n==10){ wsprintf(text1,"%d- %s",n, "I.Knight: "); }
		if (n==11){ wsprintf(text1,"%d- %s",n, "Salamander: "); }
		if (n==12){ wsprintf(text1,"%d- %s",n, "G.Ferea: "); }
		if (n==13){ wsprintf(text1,"%d- %s",n, "Evomon: "); }
		if (n==14){ wsprintf(text1,"%d- %s",n, "Chicken: "); }
		if (n==15){ wsprintf(text1,"%d- %s",n, "P.Archeron: "); }
		if (n==16){ wsprintf(text1,"%d- %s",n, "Demons: "); }
		if (n==17){ wsprintf(text1,"%d- %s",n, "Kudnum: "); }
		if (n==18){ wsprintf(text1,"%d- %s",n, "Selupan: "); }
		if (n==19){ wsprintf(text1,"%d- %s",n, "L.Silvester: "); }
		if (n==20){ wsprintf(text1,"%d- %s",n, "C.Magriffy: "); }
		if (n==21){ wsprintf(text1,"%d- %s",n, "L.Ferea: "); }
		if (n==22){ wsprintf(text1,"%d- %s",n, "Nix: "); }
		if (n==23){ wsprintf(text1,"%d- %s",n, "G.Darkness: "); }
		if (n==24){ wsprintf(text1,"%d- %s",n, "Lugard: "); }
		if (n==25){ wsprintf(text1,"%d- %s",n, "Arka War: "); }
