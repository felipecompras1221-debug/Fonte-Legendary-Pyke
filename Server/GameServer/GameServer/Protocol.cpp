...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		int valor = gScriptLoader.OnCharacterMove(lpObj->Index);
		if (valor > 0)
		{
			gEffectManager.AddEffect(lpObj, 0, 61, valor, 0, 0, 0, 0);
		}
	}
...
void GCTimeLimitSend(LPOBJ lpObj, int aIndex, DWORD Tempo) // OK
{
	PMSG_TIME_LIMIT_SEND pMsg;
...
void GCNewMessageSend2(int aIndex, int type, int color, char* message, ...) // OK
{
#if(GAMESERVER_EXTRA==1)
