...
void EntryProc(HINSTANCE hins) // OK
{
	...
	if(GetLastError() == ERROR_ALREADY_EXISTS)
	{
		...
		wsprintf(path,"%s\\GameGuard.ini",ConvertModuleFileName(name));

		GetPrivateProfileString("GameGuard","CustomerName","",CustomerName,sizeof(CustomerName),path);

		EncDecKey1 = GetPrivateProfileInt("GameGuard","EncDecKey1",0,path);

		EncDecKey2 = GetPrivateProfileInt("GameGuard","EncDecKey2",0,path);

		InternalVersion = GetPrivateProfileInt("GameGuard","InternalVersion",0,path);
