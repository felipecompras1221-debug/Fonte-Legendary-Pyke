...
void CItemOption::InsertOption(LPOBJ lpObj, CItem* lpItem, bool flag) // OK
{
	for (int n = 0; n < MAX_SPECIAL; n++)
	{
		...
		case ITEM_OPTION_ADD_TRIPLE_DAMAGE_RATE:
			...
			break;
		case ITEM_RESIST_DOUBLE_DAMAGE_RATE:
			lpObj->DoubleDamageRate += value;
			break;
		case ITEM_RESIST_IGNORE_DEFENSE_RATE:
			lpObj->ResistIgnoreDefenseRate += value;
			break;
		case ITEM_RESIST_IGNORE_SHIELD_RATE:
			lpObj->ResistIgnoreShieldGaugeRate += value;
			break;
		case ITEM_RESIST_CRITICAL_DAMAGE_RATE:
			lpObj->ResistCriticalDamageRate += value;
			break;
		case ITEM_RESIST_EXCELLENT_DAMAGE_RATE:
			lpObj->ResistExcellentDamageRate += value;
			break;
		case ITEM_RESIST_STUN_RATE:
