...
void GDSetCoinSend2(int aIndex, int value1, int value2, int value3, int value4, char* LogName) // OK
{
	LPOBJ lpObj = &gObj[aIndex];

	SDHP_SETCOIN_SEND2 pMsg;
	...
	{
		lpObj->Ruud += value4;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(912), value4);
	}
	...
	{
		lpObj->Ruud += value4;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(913), value4);
	}
	...
	LogAdd(LOG_ALERT, "[%s][%s] WZ_SetCoin(%s) Val1: %d, Val2: %d, Val3: %d, Val4: %d", lpObj->Account, lpObj->Name, LogName, value1, value2, value3, value4);
}
