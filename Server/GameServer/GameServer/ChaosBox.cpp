...
void CChaosBox::CustomMix(LPOBJ lpObj, int HeadCode) // OK
{
	CUSTOM_MIX* CustomMix = gCustomMix.ContagemMix(HeadCode);
	...
	if (CustomMix != 0)
	{
		...
		int itemMixCount = 0; int RenderGroup = -1; int ItemBagMixCount = 0;
		int ItemMixGroupCount = CustomMix->m_CountItem - gCustomMix.GruposMix(HeadCode);

		for (int slot = 0; slot < CHAOS_BOX_SIZE; slot++) { if (lpObj->ChaosBox[slot].IsItem() == 0) { continue; } if (gCustomMix.istItemBagMix(HeadCode, lpObj->ChaosBox[slot])) { ItemBagMixCount++; } }
		for (int Group = 0; Group < CustomMix->m_CountGroup; Group++)
		{
			itemMixCount = 0;
			for (int slot = 0; slot < CHAOS_BOX_SIZE; slot++)
			{
				if (lpObj->ChaosBox[slot].IsItem() == 0) { continue; }
				if (gCustomMix.istItemMix(CustomMix->m_Index, Group, lpObj->ChaosBox[slot])) { itemMixCount++; }
			}
			if (itemMixCount == ItemMixGroupCount) { RenderGroup = Group; Group = CustomMix->m_CountGroup; }
		}
		if ((itemMixCount + ItemBagMixCount) != CustomMix->m_CountItem) { this->GCChaosMixSend(lpObj->Index, 7, 0); return; }
		if (this->GetTalismanOfLuckRate(lpObj, &lpObj->ChaosSuccessRate) == 0) { this->GCChaosMixSend(lpObj->Index, 240, 0); return; }
		...
		int TaxMoney = (lpObj->ChaosMoney * gCastleSiegeSync.GetTaxRateChaos(lpObj->Index)) / 100;
		lpObj->ChaosMoney += TaxMoney;
		if (lpObj->Money < ((DWORD)lpObj->ChaosMoney)) { this->GCChaosMixSend(lpObj->Index, 2, 0); return; }
		lpObj->Money -= lpObj->ChaosMoney;
		GCMoneySend(lpObj->Index, lpObj->Money);
		gCastleSiegeSync.AddTributeMoney(TaxMoney);
		...
		if ((GetLargeRand() % 100) < lpObj->ChaosSuccessRate)
		{
			...
			RandomManager.GetRandomElement(&ItemIndex);
			CUSTOM_MIX_RESULT* ItemCreate = gCustomMix.ResultadoMix(HeadCode, RenderGroup, ItemIndex);
			if (CustomMix->m_OpcionExeRand == 0) { ItemExcellent = (ItemCreate->m_Excelent < 0) ? 0 : ItemCreate->m_Excelent; }
			else
			{
			...
			if (ItemCreate->m_Opcion > 7) { ItemOption = 7; }
			else if (ItemCreate->m_Opcion < 0) { ItemOption = 0; }
			else { ItemOption = ItemCreate->m_Opcion; }
