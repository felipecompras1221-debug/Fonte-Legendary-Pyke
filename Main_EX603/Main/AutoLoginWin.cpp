...
BYTE DrawCButton(int IMGcode, float x, float y, float w, float h, float a6 = 64.0, float a7 = 128.0, int delay = 150, bool a9 = false)
{
	if (IsCursorInZone(x, y, getX(w), getY(h))) 
	{
		...
		{
			aRenderBitmap(IMGcode, x, y, w, h, 0.0, h / a7 * 2.0, w / a6, h / a7, 0, 1, 0.0);
		}
		else 
		{
...
void CButtonAutoLogin(int This, int a2)
{
	...
	if (!AutoLogin.GetShowListAccount(false))
	{
		if (IsCursorInZone(startX + getX(250.0), startY + getY(155), getX(15), getY(15))) 
		{
			...
			{
				aRenderBitmap(0x7B69, startX + getX(250.0), startY + getY(155), 15.0, 15.0, 0.0, 0.0, 15.0 / 16.0, 15.0 / 32.0, 0, 1, 0.0);
		...
		if (!AutoLogin.savePass) 
		{
			aRenderBitmap(0x7B69, startX + getX(250.0), startY + getY(155), 15.0, 15.0, 0.0, 15.0 / 32.0, 15.0 / 16.0, 15.0 / 32.0, 0, 1, 0.0);
		}
		else 
		{
			aRenderBitmap(0x7B69, startX + getX(250.0), startY + getY(155), 15.0, 15.0, 0.0, 0.0, 15.0 / 16.0, 15.0 / 32.0, 0, 1, 0.0);
	...
	CustomFont.Draw((HFONT)pFontNormal, startX + getX(129), startY + getY(158), 0xffffffD1, 0x0, getX(123), 0, 4, "%s", "Save Account/Password");
	...
	if (AutoLogin.GetShowListAccount(false)) 
	{
		...
		aRenderBitmap(31322, startX + getX(109), startY + getY(128), 155.0, 102.0, 0.0, 36.0 / 256.0, 112.0 / 128.0, 144.0 / 256.0, 0, 1, 0.0);
		
		for (i = 1; i < 5; i++) 
		{
			aRenderBitmap(31582, startX + getX(109), startY + getY(128 + 20 * i), 155.0, 2.0, 0.0, 0.0, 82.0 / 128.0, 2.0 / 2.0, 0, 1, 0.0);
		}
		...
		aRenderBitmap(31579, startX + getX(109), startY + getY(128), 155.0, 20.0, 0.0, 0.0, 112.0 / 128.0, 44.0 / 64.0, 0, 1, 0.0);
		
		for (i = 0; i < 8; i++) 
		{
			aRenderBitmap(31580, startX + getX(109), startY + getY(148 + 9.0 * i), 155.0, 10.0, 0.0, 0.0, 112.0 / 128.0, 14.5 / 16.0, 0, 1, 0.0);
		}
		
		aRenderBitmap(31581, startX + getX(109), startY + getY(212.5), 155.0, 20.0, 0.0, 0.0, 112.0 / 128.0, 44.0 / 64.0, 0, 1, 0.0);
	}
}
