...
void CCustomMix::Load(char* path) {
	...
	if (lpMemScript->SetBuffer(path) == 0) { ErrorMessageBox(lpMemScript->GetLastError()); delete lpMemScript; return; }
	...
	for (int i = 0; i < 254; i++) { this->ResultadoSomaivos[i].MixeSs = -1; this->ResultadoSomaivos[i].m_ResultadoSomaivos.clear(); }

	try
	{
		while (true)
		{
			if (lpMemScript->GetToken() == TOKEN_END) { break; }
			...
			while (true)
			{
				if (section == 0)
				{
					if (strcmp("end", lpMemScript->GetAsString()) == 0) { break; }
				...
				else if (section == 3)
				{
					...
					this->ResultadoSomaivos[info3.m_Index].m_ResultadoSomaivos.insert(std::pair<int, CUSTOM_MIX_RESULT>(info3.m_Group, info3));
...
CUSTOM_MIX* CCustomMix::ContagemMix(int IndexMix)
{
	CUSTOM_MIX* value;
	std::map<int, CUSTOM_MIX>::iterator it = this->m_Data_Mix.find(IndexMix);
	if (it != this->m_Data_Mix.end()) { value = &it->second; return value; }
	return 0;
}

CUSTOM_MIX_RESULT* CCustomMix::ResultadoMix(int IndexMix, int group, int ItemIndex)
{
	std::map<int, CUSTOM_MIX_RESULT>::iterator it = this->ResultadoSomaivos[IndexMix].m_ResultadoSomaivos.find(group);
	if (it != this->ResultadoSomaivos[IndexMix].m_ResultadoSomaivos.end()) { if (ItemIndex >= it->second.m_ItemIndexMin && ItemIndex <= it->second.m_ItemIndexMax) { return &it->second; } }
	return 0;
}
int CCustomMix::GruposMix(int IndexMix)
{
	int itemcount = 0;
	for (std::map<int, CUSTOM_BAG_MIX>::iterator it = this->m_Data_Bag.begin(); it != this->m_Data_Bag.end(); it++) { if (it->second.m_Index == IndexMix) { itemcount += it->second.m_Count; } }
	return itemcount;
}

int CCustomMix::ContItensMix(int IndexMix, int Group)
{
	int itemcount = 0;
	for (std::map<int, CUSTOM_ITEM_MIX>::iterator it = this->m_Data_Item.begin(); it != this->m_Data_Item.end(); it++) { if (it->second.m_Index == IndexMix && it->second.m_Group == Group) { itemcount += it->second.m_Count; } }
	return itemcount;
}
...
CRandomManager CCustomMix::RandomManager(int IndexMix, int group) {
	CRandomManager RandomManager;
	std::map<int, CUSTOM_MIX_RESULT>::iterator it = this->ResultadoSomaivos[IndexMix].m_ResultadoSomaivos.find(group);
	if (it != this->ResultadoSomaivos[IndexMix].m_ResultadoSomaivos.end()) { for (int n = it->second.m_ItemIndexMin; n <= it->second.m_ItemIndexMax; n++) { RandomManager.AddElement(n, 1); } }
