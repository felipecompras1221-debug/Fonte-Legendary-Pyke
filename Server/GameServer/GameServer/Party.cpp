...
void CParty::PartyMoveUser(LPOBJ lpObj, int number)
{
	if (gServerInfo.m_PartyTeleportSwitch == 0)
	{
		return;
	}
	//LPOBJ lpObj			= &gObj[OldLeader];
	...
	if (lpObj->Money < gServerInfo.m_PartyTeleportMoney)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(498));
	}

	lpObj->Money -= gServerInfo.m_PartyTeleportMoney;
